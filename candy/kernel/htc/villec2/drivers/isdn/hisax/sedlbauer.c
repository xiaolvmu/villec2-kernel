...
{"None", "speed card/win", "speed star", "speed fax+",
 ...
 "speed fax+ villec2", "speed fax+ pci", "HST Saphir III"};

#define PCI_SUBVENDOR_SPEEDFAX_VILLEC2	0x51
...
#define SEDL_SPEEDFAX_VILLEC2	7
...
Sedl_card_msg(struct IsdnCardState *cs, int mt, void *arg)
{
	...
	case CARD_TEST:
		return (0);
	case MDL_INFO_CONN:
		if (cs->subtyp != SEDL_SPEEDFAX_VILLEC2)
			return (0);
		spin_lock_irqsave(&cs->lock, flags);
		if ((long) arg)
...
setup_sedlbauer_pci(struct IsdnCard *card)
{
	...
	if (sub_vendor_id == PCI_SUBVENDOR_SPEEDFAX_VILLEC2) {
		cs->hw.sedl.chip = SEDL_CHIP_ISAC_ISAR;
		cs->subtyp = SEDL_SPEEDFAX_VILLEC2;
