...
#include "board-villec2-audio-data.h"
...
static uint32_t msm_aic3254_reset_gpio[] = {
	GPIO_CFG(VILLEC2_AUD_CODEC_RST, 0, GPIO_CFG_OUTPUT,
...
void villec2_snddev_poweramp_on(int en)
{
	...
	if (en) {
		msleep(50);
		gpio_direction_output(PM8058_GPIO_PM_TO_SYS(VILLEC2_AUD_HP_EN), 1);
		set_speaker_amp(1);
		if (!atomic_read(&aic3254_ctl))
			curr_rx_mode |= BIT_SPEAKER;
	} else {
		set_speaker_amp(0);
		gpio_direction_output(PM8058_GPIO_PM_TO_SYS(VILLEC2_AUD_HP_EN), 0);
		if (!atomic_read(&aic3254_ctl))
			curr_rx_mode &= ~BIT_SPEAKER;
	}
}
...
void villec2_snddev_bmic_pamp_on(int en)
{
	...
	if (en) {
		...
		gpio_direction_output(PM8058_GPIO_PM_TO_SYS(VILLEC2_AUD_MIC_SEL), 0);
	} else {
...
void villec2_snddev_emic_pamp_on(int en)
{
	...
	if (en) {
		...
		gpio_direction_output(PM8058_GPIO_PM_TO_SYS(VILLEC2_AUD_MIC_SEL), 1);
	}
}
...
void villec2_snddev_stereo_mic_pamp_on(int en)
{
	...
	if (en) {
		...
		/* select external mic path */
		gpio_set_value(PM8058_GPIO_PM_TO_SYS(VILLEC2_AUD_MIC_SEL), 0);
	} else {
		ret = pm8058_micbias_enable(OTHC_MICBIAS_0, OTHC_SIGNAL_OFF);
		if (ret)
...
void villec2_snddev_fmhs_pamp_on(int en)
{
	pr_info("%s %d\n", __func__, en);
	if (en) {
		gpio_set_value(PM8058_GPIO_PM_TO_SYS(VILLEC2_AUD_HP_EN), 1);
		set_headset_amp(1);
		if (!atomic_read(&aic3254_ctl))
			curr_rx_mode |= BIT_FM_HS;
	} else {
		set_headset_amp(0);
		gpio_set_value(PM8058_GPIO_PM_TO_SYS(VILLEC2_AUD_HP_EN), 0);
		if (!atomic_read(&aic3254_ctl))
			curr_rx_mode &= ~BIT_FM_HS;
	}
}
...
void villec2_rx_amp_enable(int en)
{
	if (curr_rx_mode != 0) {
		...
		if (curr_rx_mode & BIT_RECEIVER)
			villec2_snddev_receiver_pamp_on(en);
		if (curr_rx_mode & BIT_FM_SPK)
			villec2_snddev_fmspk_pamp_on(en);
		if (curr_rx_mode & BIT_FM_HS)
			villec2_snddev_fmhs_pamp_on(en);
...
void villec2_get_acoustic_tables(struct acoustic_tables *tb)
{
	switch (system_rev) {
	case 0:
	case 1:
...
void villec2_reset_3254(void)
{
	gpio_tlmm_config(msm_aic3254_reset_gpio[0], GPIO_CFG_ENABLE);
	gpio_set_value(VILLEC2_AUD_CODEC_RST, 0);
	mdelay(1);
	gpio_set_value(VILLEC2_AUD_CODEC_RST, 1);
}
...
static struct aic3254_ctl_ops cops = {
	.rx_amp_enable        = villec2_rx_amp_enable,
	.reset_3254           = villec2_reset_3254,
...
static struct q6v2audio_aic3254_ops aops = {
       .aic3254_set_mode = villec2_aic3254_set_mode,
};

void villec2_audio_gpios_init(void)
{
	pr_info("%s\n", __func__);
	gpio_request(PM8058_GPIO_PM_TO_SYS(VILLEC2_AUD_HP_EN), "AUD_SPK_ENO");
	gpio_request(PM8058_GPIO_PM_TO_SYS(VILLEC2_AUD_MIC_SEL), "AUD_HANDSET_ENO");
}
