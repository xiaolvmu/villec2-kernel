...
_dhd_sysioc_thread(void *data)
{
	...
	set_wifi_foot_print(0x1, 0x08);
		...
		set_wifi_foot_print(0x2, 0x08);
		dhd_net_if_lock_local(dhd);
		DHD_OS_WAKE_LOCK(&dhd->pub);
		set_wifi_foot_print(0x3, 0x08);

		for (i = 0; i < DHD_MAX_IFS; i++) {
			if (dhd->iflist[i]) {
				set_wifi_foot_print(0x4, 0x08);
		...
		set_wifi_foot_print(0x6, 0x08);
		DHD_OS_WAKE_UNLOCK(&dhd->pub);
		dhd_net_if_unlock_local(dhd);
		set_wifi_foot_print(0x7, 0x08);
	}
	DHD_TRACE(("%s: stopped\n", __FUNCTION__));
...
dhd_watchdog_thread(void *data)
{
	...
	complete(&tsk->completed);
#endif
	set_wifi_foot_print(0x1, 0x0C);
	while (1)
		if (down_interruptible (&tsk->sema) == 0) {
			unsigned long flags;
			set_wifi_foot_print(0x2, 0x0C);
			if (dhd->dhd_force_exit== TRUE)
				break;
			...
			if (dhd->pub.dongle_reset == FALSE) {
				...
				dhd_os_spin_unlock(&dhd->pub, flags);
				set_wifi_foot_print(0x5, 0x0C);
			}
			dhd_os_sdunlock(&dhd->pub);
			set_wifi_foot_print(0x6, 0x0C);
			DHD_OS_WAKE_UNLOCK(&dhd->pub);
			set_wifi_foot_print(0x7, 0x0C);
		} else {
			break;
	}

	set_wifi_foot_print(0x8, 0x0C);
	complete_and_exit(&tsk->completed, 0);
}
#endif 
...
static void set_wifi_foot_print_time(unsigned state, unsigned int offset)
{
...
dhd_dpc_thread(void *data)
{
	...
	while (1) {
		...
		timec = ktime_to_ns(ktime_get());
		timea = timec >> 32;
		timeb = timec & 0xFFFFFFFF;
		set_wifi_foot_print_time(timea, 0x50);
		set_wifi_foot_print_time(timeb, 0x54);
		
		set_wifi_foot_print(0x2, 0x30);
		if (down_interruptible(&tsk->sema) == 0) {
			set_wifi_foot_print(0x3, 0x30);
			
			timec = ktime_to_ns(ktime_get());
			timea = timec >> 32;
			timeb = timec & 0xFFFFFFFF;
			set_wifi_foot_print_time(timea, 0x60);
			set_wifi_foot_print_time(timeb, 0x64);
			...
			set_wifi_foot_print(0x4, 0x30);
			
			if (dhd->pub.busstate != DHD_BUS_DOWN) {
				if (dhd_bus_dpc(dhd->pub.bus)) {
					set_wifi_foot_print(0x5, 0x30);
					up(&tsk->sema);
					set_wifi_foot_print(0x6, 0x30);
				}
				else {
					DHD_OS_WAKE_UNLOCK(&dhd->pub);
				}
			} else {
				set_wifi_foot_print(0x7, 0x30);
				if (dhd->pub.up)
					dhd_bus_stop(dhd->pub.bus, TRUE);
				DHD_OS_WAKE_UNLOCK(&dhd->pub);
			}
			set_wifi_foot_print(0x8, 0x30);
		}
		else
			break;
	}

	set_wifi_foot_print(0x9, 0x30);
	complete_and_exit(&tsk->completed, 0);
}
#endif 
